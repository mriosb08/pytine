...
def main(args):
    (parc_file, score_file, task, senna_path, tt_path) = args
    preprocess(parc_file, score_file, task, senna_path, tt_path)
    return

def preprocess(parc_file = '', score_file = '', task = 'sts', senna_path = '/media/raid-vapnik/tools/senna/', tt_path = '/home/tools/treetagger/'):
    entailment_pairs = []
...
def load_scores(score_file = ''):
    ...
    with open(score_file, 'r') as f:
        for line in f:
            line = line.rstrip('\n')
            #score line example: id="1" value="TRUE" task="parc"
            m = re.search(r'\svalue=\"([A-Z]+)\"\s',line)
            if m:
                value = m.group(1)
            else:
                value = 'FALSE'
            score_queue.append(value)
    return score_queue
...
if __name__ == "__main__":
    if len(sys.argv[1:]) != 5:
        print 'USAGE: ./preprocessing_parc.py <parc-file> <score-file> <task> <senna-path> <tt-path>'
        sys.exit(0)
    else:
        main(sys.argv[1:])
